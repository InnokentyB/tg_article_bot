...
API умеет собирать ежедневный редакционный дайджест: 5 лучших статей за последние 3 дня с кратким русским пояснением по каждой, одну отдельную статью дня и критический разбор для канала «Читатель Use Case».
...
без статьи дня, затем отдельный разбор статьи дня. Посты не показывают дату отбора; источник
не выводится отдельной строкой, если он уже виден из заголовка или ссылки. Задержка перед вторым постом задается
