...
API умеет собирать ежедневный редакционный дайджест: 5 лучших статей за последние 3 дня с кратким пояснением по каждой, одну отдельную статью дня и критический разбор для канала «Читатель Use Case».
...
без статьи дня, затем отдельный разбор статьи дня. Задержка перед вторым постом задается
